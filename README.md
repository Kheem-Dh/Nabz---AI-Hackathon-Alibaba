...
This drop lands the "adaptive triage + curated evidence + doctor-ready dashboard" phase of the winning plan. The live clinical interview is generated by Qwen; pytest validates the API/model boundary and `eval.py` validates the surrounding safety gates without recreating a clinical rules engine.

### ✅ Shipped in this pass

**Adaptive clinical interview**

- Qwen now generates a `ClinicalState` snapshot (chief complaint, body location + laterality, onset/duration, course, severity, associated symptoms, red flags present/denied, and unknowns) from the full transcript on every turn. The server validates and returns it on every `TriageTurn`.
...
- Qwen identifies emergency features from the complete transcript; the server enforces EMERGENCY whenever the model reports a red flag. The five-question ceiling and conservative, visibly labelled AI-unavailable fallback remain server-enforced.
...
- Live triage generates follow-up questions from the complaint, complete transcript,
  and relevant Vault context instead of selecting from complaint-specific paths.
...
- Browser speech recognition continues through short pauses, safely restarts
  if the browser ends a continuous session, and uses an eight-second initial
  silence window plus explicit transcript review before sending.
- A breathing-difficulty answer is interpreted in the context of the exact
  preceding question by the live model rather than by a yes/no keyword rule.
...
| `MOCK_MODE`         | Enable synthetic lab/prescription/demo fixtures; triage still requires live AI |
...
Mock mode is intended for reliable document, lab, prescription, and Vault fixtures. It does not provide canned clinical triage. Without a working DashScope key, triage returns a clearly labelled AI-unavailable safety response.

- lab-report extraction;
- prescription extraction;
- low-confidence prescription fields.

Synthetic non-triage responses include:

```json
{ "mock": true }
```

Triage responses instead expose `response_source` as `live_ai`,
`ai_unavailable`, or the pytest-only `test_model`.
